 ...
 * 检测 Node.js 测试框架
 */
function detectNodeTestFrameworks(
  projectRoot: string,
 ...
 * 检测 Python 测试框架
 */
function detectPythonTestFrameworks(projectRoot: string, frameworks: TestFrameworkInfo[]): void {
 ...
 * 检测 Java 测试框架
 */
function detectJavaTestFrameworks(projectRoot: string, frameworks: TestFrameworkInfo[]): void {
 ...
 * 检测测试风格
 */
export function detectTestStyle(projectRoot: string, existingTests: ExistingTestFile[]): TestStyle | undefined {
 ...
 * 检测是否有前端项目
 */
export function detectFrontend(projectRoot: string, projectType: ProjectType): FrontendInfo {
 ...
 * 检测覆盖率报告
 */
export function detectCoverageReport(projectRoot: string): CoverageReport | undefined {
  const coveragePaths = [
 ...
 * 执行完整扫描并返回结果
 */
export function performFullScan(projectRoot: string, target?: string) {
  // 确定扫描目录
