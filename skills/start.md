...
while (有待执行任务) {
  ...
  6. **检查是否需要审批**:
     - 如果配置了 auto 模式 (`approvalPoints` 为空): 自动批准，继续执行
     - 如果遇到 meeting 类型审批: 暂停（即使是 auto 模式）
     - 如果有非 meeting 类型 pending 审批: 自动批准（auto 模式）
     - 其他情况: 暂停等待人工确认
  7. 继续下一个任务
}
```

**⚠️ 重要**: 在 **auto 模式** 下，Skill **不得**询问用户"是否继续执行"、"是否执行下一 Phase"等问题。遇到非审批类确认时，应**自动继续**。
