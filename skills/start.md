...
while (有待执行任务) {
  ...
  6. **检查是否需要审批**:
     - 如果配置了 auto 模式 (`approvalPoints` 为空): **自动批准所有审批（包括 meeting）**，继续执行，记录待最后统一查看
     - 如果配置了非 auto 模式: 在配置的审批点暂停等待确认
     - 其他情况: 暂停等待人工确认

**Meeting 处理**: 即使是 meeting 类型，在 auto 模式下也自动批准，记录到状态文件供最后统一查看，不中断执行流程
  7. 继续下一个任务
}
```
...
- 不得询问"是否执行下一 Phase"
- 不得询问"是否处理 Meeting"
- **所有审批（包括 meeting）自动批准并记录，最后统一汇总展示**
