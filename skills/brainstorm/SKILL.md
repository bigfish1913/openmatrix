...
## Step 5.5: 前端原型生成（可选但强烈推荐）

**⚠️ 重要：如果任务涉及任何可见的用户界面，必须执行此步骤。**

### 5.5.1 强制检测是否涉及前端

**不要跳过此检测。** 以下信号**任一出现**即触发：

| 信号类型 | 关键词示例 |
|---------|-----------|
| **Web 应用** | 网站、网页、Web、在线、浏览器 |
| **UI 相关** | UI、界面、页面、布局、样式、CSS |
| **交互元素** | 表单、按钮、导航、菜单、弹窗、对话框、卡片 |
| **数据可视化** | 图表、Dashboard、仪表盘、统计图、表格展示 |
| **前端框架** | React、Vue、Angular、Next.js、Svelte、HTML、CSS |
| **移动端** | App、iOS、Android、移动端、手机端 |
| **管理后台** | 后台、管理系统、CRM、ERP、控制台 |
| **文件类型** | .html、.vue、.tsx、.jsx、.css、.scss |
| **隐含场景** | 用户登录、注册、查看数据、填写信息 |

**隐性前端场景（不要漏掉）：**
- "实现用户登录功能" → 必须有登录页面（前端）
- "添加数据导出功能" → 如果导出前有预览界面（前端）
- "优化用户体验" → UI 优化（前端）
- "修改某个页面" → 明确提到页面（前端）

**强制检测逻辑：**

在 Step 5 完成后，**必须执行以下判断**：

```
前端检测 = 任务描述.includes(前端关键词) ||
           设计涉及用户交互界面 ||
           输出产物包含 HTML/Vue/React 文件
```
...
**question:** 🎨 **检测到前端/UI 相关内容！** 此任务涉及用户界面设计。是否生成 HTML 原型作为可视化参考？

| label | description |
|-------|-------------|
| 生成原型 (强烈推荐) | 消耗 5-10k tokens，获得可视化原型指导开发 |
| 不生成原型 | 节省 token，仅用文字描述（需对设计非常熟悉） |
