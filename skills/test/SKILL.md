---
name: om:test
description: "Use to GENERATE or IMPROVE tests. Triggers on TEST-CREATION intent: generate test files, improve coverage, test infrastructure setup. DO NOT trigger on: running tests, debugging test failures, or test questions. Auto-validates tests, loops up to 3 times on failure."
priority: high
---
...
- ✅ **内部模块调用用真实实现**（如 A 调用 B，测试完整链路）
- ✅ **文件系统用临时目录**（`os.tmpdir()` 或 `fs.promises.mkdtemp`）
- ✅ **内存数据库用真实连接**（如 SQLite in-memory、Prisma test DB）
- ❌ **不要 mock 被测模块的内部依赖**（那等于没测）
- ❌ **不要 mock 内部函数返回值**（测试真实行为）
...
| 情况 | 处理方式 |
|------|---------|
| 测试 utils 函数 | ✅ 不 mock，用真实输入输出 |
| 测试 service 调用 repository | ✅ 用真实 repository（内存数据库） |
| 测试 API 调用第三方服务 | ❌ 可以 mock HTTP 请求 |
| 测试文件处理 | ✅ 用临时文件，不 mock fs |
| 测试数据库操作 | ✅ 用测试数据库（如 test DB），不 mock ORM |
...
❌ **Mock 被测模块的内部依赖**（那等于没测）
❌ **Mock 内部函数/模块的返回值**（测试假数据）
❌ 写 \`expect(true).toBe(true)\` 这样的空测试`,
...
1. 如果是测试语法错误 → 修正测试代码
2. 如果是断言失败 → 分析预期值是否正确
3. 如果是 Mock 问题 → **优先移除不必要的 mock，改用真实依赖**
